...
mutual
  ...
  cgBody ret (SApp True f args) = do
    ...
    -- non-self tail call
      ...
      (retTy, _) <- getFunctionType targetMethodName
      if (retTy == Ref thunkClass) then
         -- already returns a thunk, just call the function in tail call position
         cgSApp ret True f args
      else do
